...
bool KinematicFitInterface::doFit(std::vector<reco::TransientTrack> t_tracks, const double muonMass, const double mass1, const double  mass2){
  ...
  reco::TransientTrack track_Pim = t_tracks[3];
  ...
  allParticlesTrk.push_back(pFactory.particle (track_Pim, mass2, chi, ndf, pi_sigma));
  ...
  myTree_Bd = Fitter.fit(allParticlesTrk);
  
  // if the fit fails, do not consider this as candidate
  if(myTree_Bd->isEmpty()) {
    delete jpsi_const;
    return 0;
  }
  
  myTree_Bd->movePointerToTheTop();
 
  bhadron = myTree_Bd->currentParticle();
  bVertex = myTree_Bd->currentDecayVertex();
